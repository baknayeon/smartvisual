...
def unsupportedDevicesPage() {
	...
	dynamicPage(name: "unsupportedDevicesPage") {
		if (unsupportedDimmers) {
			...
			section {
				input(name: "dimmers_sensor", type: "capability.sensor", title: "Please remove the above devices from this list.", submitOnChange: true, multiple: true)
			}
			section {
	...
	dimmer3.each { dimmer ->
		...
		if (nextLevel == 0) {
			...
			if (shouldChangeColors && hasSetColorCommand(dimmer)) {
				...
				dimmer2.setColor([hue: hue, saturation: 100, level: nextLevel]) //colorControl
